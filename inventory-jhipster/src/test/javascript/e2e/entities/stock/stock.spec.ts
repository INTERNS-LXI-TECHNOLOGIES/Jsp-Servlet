...
describe('Stock e2e test', () => {
  ...
  it('should load Stocks', async () => {
    ...
    expect(await stockComponentsPage.getTitle()).to.eq('Stocks');
  });

  it('should load create Stock page', async () => {
    await stockComponentsPage.clickOnCreateButton();
    stockUpdatePage = new StockUpdatePage();
    expect(await stockUpdatePage.getPageTitle()).to.eq('Create or edit a Stock');
  ...
  it('should create and save Stocks', async () => {
    ...
    await stockComponentsPage.clickOnCreateButton();
    await promise.all([stockUpdatePage.setQuantityInput('5'), stockUpdatePage.productSelectLastOption()]);
  ...
  it('should delete last Stock', async () => {
    ...
    expect(await stockDeleteDialog.getDialogTitle()).to.eq('Are you sure you want to delete this Stock?');
